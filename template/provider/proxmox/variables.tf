variable "pm_api_url" {
  default = "{{config.get_value('proxmox', 'pm_api_url', 'https://192.168.150.200:8006/api2/json')}}"
}
...
variable "pm_node" {
  default = "{{config.get_value('proxmox', 'pm_node', 'pvs')}}"
}
...
variable "vm_template_id" {
  ...
  default = {
      "WinServer2019_x64"  = {{config.get_value('proxmox_templates_id', 'WinServer2019x64-cloudinit', 129)}}
      "WinServer2019_x64_utd"  = {{config.get_value('proxmox_templates_id', 'WinServer2019_x64_utd', 201901)}}
      "WinServer2016_x64"  = {{config.get_value('proxmox_templates_id', 'WinServer2016x64-cloudinit', 128)}}
...
variable "storage" {
  ...
  default = "{{config.get_value('proxmox', 'pm_storage', 'Data')}}"
}

variable "network_bridge" {
  default = "{{config.get_value('proxmox', 'pm_network_bridge', 'vmbr0')}}"
}
